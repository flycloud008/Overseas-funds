	...
	<view class="container">
		<view class="main">
			<view class="income-card" style="margin-top: 0upx;">
				...
				<view class="show-content">
					<view class="view-category">
						<view class="num"><text style="font-size: 24upx;">{{member.jindou}}个</text></view>
						<view class="text">消费积分</view>
					</view>
				
					<view class="line"></view>
					<view class="view-category">
						<view class="num"> <text style="font-size: 24upx;">{{member.score}}个</text> </view>
						<view class="text">商城积分</view>
					</view>
				</view>
			</view>
			<!-- 我的团队 -->
			<view class="income-card">
				...
				<view class="show-input">
					...
					<u-field v-model="targetNum" label="兑换数量" placeholder="请填兑换数量" />
					<u-button type="warning" style="width: 100%;" @click="submit">立即兑换</u-button>
				</view>
			</view>
			<!-- 工具 -->
			<view class="tool-card">
				<view class="tool-item"
					@click="navTo('/pages/index/accountList',{title:'鸡蛋兑换记录',moneyType:JSON.stringify(['3','4','2','10']),changeType:JSON.stringify(['零售鸡蛋兑换积分','批发鸡蛋兑换积分'])})">
					<u-icon name="man-add" size="45" color="#FF0000"></u-icon>
					<view class="item-right">

						<view class="view-text">鸡蛋兑换记录</view>
	...
	export default {
		...
		methods: {
			...
			async submit() {
				...
				var res = await this.$u.post('/work_ddl/user/duihuanJiFen', {
					typevalue: this.typevalue,
					targetNum: this.targetNum
				});

				if (res.result == 'true') {
					uni.$u.toast('兑换' + this.typevalue + '成功');
					setTimeout(function() {
						uni.navigateBack();
					}, 1500)
				}
