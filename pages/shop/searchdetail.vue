<template>
	<view class="content">
		<view class="search-box">
			...
			<mSearch class="mSearch-input-box" :value2="defaultKeyword"  :mode="2" button="inside" :placeholder="defaultKeyword"
	...
	export default {
		...
		methods: {
			...
			async loadDefaultKeyword() {
				...
				let searchs = await this.$u.post("/html/plug_shop/shopindex/getShopSearchList");
				this.defaultKeyword ="热门："+searchs.none;
			},
			...
			async loadHotKeyword() {
				...
				let search = await this.$u.post("/html/plug_shop/shopindex/getShopSearchList");
				this.hotKeywordList = search.list
			}, 
			//监听输入
			inputChange(event) {
			...
			doSearch(keyword) {
				...
				uni.navigateTo({
					url: `/pages/shop/list?key=${keyword}`
				})
			},
